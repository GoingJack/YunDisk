    ...
    ui(new Ui::ButtonGroup)
{
    ...
    //设置用户名
    connect(ui->login_usr,&QToolButton::clicked,[=]()
            {
                 setUserTitle();
            }
            );
...
//设置用户名
void ButtonGroup::setUserTitle()
{
    //设置当前title用户名
    // 获取登陆信息实例
    LoginInfoInstance *login = LoginInfoInstance::getInstance(); //获取单例
    QString cur_username = login->getUser();
    ui->login_usr->setToolButtonStyle(Qt::ToolButtonTextUnderIcon);
    ui->login_usr->setText(cur_username);
}
