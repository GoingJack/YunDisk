    ...
    if (error.error == QJsonParseError::NoError) //没有出错
    {
        ...
        if( doc.isObject())
        {
            ...
            for(int i = 0; i < size; ++i)
            {
                ...
                */
                Common c_tmp;
                QString add_url = c_tmp.getCfgValue("web_server","ip","conf/cfg.json");
                QString real_url = tmp.value("url").toString();
                real_url=real_url.replace(QRegExp(IP_REG),add_url);
                ...
                info->url = real_url;/*tmp.value("url").toString(); //url*/
